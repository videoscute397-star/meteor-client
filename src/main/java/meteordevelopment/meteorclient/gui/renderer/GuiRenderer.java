...
public class GuiRenderer {
    ...
    /**
     * Draws a rectangle with rounded corners using the existing triangle() primitive.
     * radius is clamped so it never exceeds half the smaller dimension.
     * segments controls corner smoothness (6-8 looks good for small UI radii; higher = smoother but more triangles).
     */
    public void roundedQuad(double x, double y, double width, double height, double radius, Color color) {
        roundedQuad(x, y, width, height, radius, color, 8);
    }

    public void roundedQuad(double x, double y, double width, double height, double radius, Color color, int segments) {
        ...
        // Center cross: middle rect + top/bottom strips (avoids re-covering the corner arcs)
        quad(x + r, y, width - 2 * r, height, color);
        quad(x, y + r, r, height - 2 * r, color);
        quad(x + width - r, y + r, r, height - 2 * r, color);

        // Corner centers
        double tlX = x + r, tlY = y + r;
        double trX = x + width - r, trY = y + r;
        double blX = x + r, blY = y + height - r;
        double brX = x + width - r, brY = y + height - r;

        fanCorner(tlX, tlY, r, 180, 270, segments, color);
        fanCorner(trX, trY, r, 270, 360, segments, color);
        fanCorner(brX, brY, r, 0, 90, segments, color);
        fanCorner(blX, blY, r, 90, 180, segments, color);
    }

    private void fanCorner(double cx, double cy, double radius, double startDeg, double endDeg, int segments, Color color) {
        double startRad = Math.toRadians(startDeg);
        double stepRad = Math.toRadians(endDeg - startDeg) / segments;

        for (int i = 0; i < segments; i++) {
            double a1 = startRad + stepRad * i;
            double a2 = startRad + stepRad * (i + 1);
